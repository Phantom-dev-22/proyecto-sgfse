...
# --- INICIO BLOQUE INSTALACION AUTOMATICA ---
...
def crear_base_de_datos_nube():
    try:
        ...
        # 3. Crear Tabla USUARIOS (Conectada a Roles)
        cur.execute("""
            ...
            (
                ...
                CONSTRAINT fk_id_rol FOREIGN KEY (id_rol)
                    REFERENCES asistencia_sgfse."Roles" (id_rol) MATCH SIMPLE
                    ON UPDATE NO ACTION
                    ON DELETE NO ACTION
            );
        """)

        # 4. Insertar Rol de Administrador (Si no existe)
        ...
        # 5. Insertar USUARIO ADMIN (RUT: 1-9, Clave: admin123)
        # Usamos werkzeug que ya tienes importado para encriptar
        ...
        if not user:
            ...
            msg_usuario = "✅ Usuario Admin creado (RUT: 1-9 / Pass: admin123)"
        else:
            msg_usuario = "⚠️ El usuario Admin ya existía."
